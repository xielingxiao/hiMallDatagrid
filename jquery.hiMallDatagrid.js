/**
 * jQuery hiMallDatagrid 2.0
 *
 * 作者：廖日龙
 * 说明：基于JQuery表格数据初始化的插件
 * 修改完善：five(673921852)
 *
 * 
 */
(function ($) {
    ...
    function request(target, param) {
        ...
        (function initialOperationBtn() {
            //初始化操作按钮
            if ($(opts.operationButtons).length) {
                var inner = $(opts.operationButtons).html();
                $(opts.operationButtons).remove();
                opts.operationButtonsInnerHTML = inner;
            }
    ...
    function bindCellsEvents(target) {
        ...
        $("tbody tr", $(target)).unbind("dblclick").bind("dblclick", function (e) {
            var findIndex = $(this).attr("hiMallDatagrid-row-index");
            if (opts.singleSelect) {
                clearSelections(target);
                selectRow(target, findIndex);
            } else {
                if (!$(this).find("div.hiMallDatagrid-cell-check input[type=checkbox]").is(":checked")) {
                    selectRow(target, findIndex);
                } else {
                    unSelectRow(target, findIndex);
                }
    ...
    var renderView = {
        ...
        renderHead: function (target, container) {
            var opts = $.data(target, "hiMallDatagrid").options;
            var closFileds = $(target).hiMallDatagrid("getColumnFields");
            var htmlAry = ["<thead style='height:20px;'><tr>"];
